...
let handler = async (m, { usedPrefix, args, command, text }) => {
  ...
  if (args.length < 2 )  throw `✳️ ${mssg.useCmd}: *${usedPrefix + command}* <cantidad> <espacio>\n\n*${usedPrefix + command}-info* ${mssg.moreInfo}`;
  ...
  if (!validSpaces.includes(space) && (!/^\d+$/.test(space) || parseInt(space) < 1 || parseInt(space) > 36)) {
    m.reply(`✳️ ${mssg.useCmd}: *${usedPrefix + command}* <cantidad> <espace>\n\n*${usedPrefix + command}-info* ${mssg.moreInfo}`);
    return;
  }
  ...
  const color = isRedNumber(result) ? "rojo" : "negro";
  ...
  m.reply(resultMessage, null, fwc);
